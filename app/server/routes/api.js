...
module.exports = function(router) {
  ...
  router.get("/users", isAdmin, function(req, res) {
    ...
    // query.sort = 'date', query.sort = 'alphabet'
      ...
      hellosign.signatureRequest
        ...
        .then(function(response) {
          if (response.signature_requests.length) {
            signatureExists = response.signature_requests[0].signatures.find(
              signature => signature.signer_email_address === email
            );
            if (signatureExists) {
              res.send(200);
            } else {
  ...
  router.post("/users/acceptQueue", isAdmin, function(req, res) {
    ...
    UserController.getByToken(token, function(err, user) {
      ...
      UserController.acceptAllInAcceptedQueue(
        user.email,
    ...
    if (confirmation.needsReimbursement) {
      oauth2Client.refreshAccessToken((err, tokens) => {
        ...
        var email = UserController.getById(id, (error, user) => {
          if (
            user.profile.school === "University of California-Los Angeles" ||
            user.profile.school ===
          ...
          sheets.spreadsheets.values.get(
            ...
            (err, result) => {
              ...
              if (
                confirmation.needsReimbursement === "1" &&
                !result.values.some(value => {
                  return (
                    JSON.stringify(value) === JSON.stringify(user.email.split())
                  );
                })
              ) {
                sheets.spreadsheets.values.append(
                  {
                    ...
                    resource: {
                      values: [[new Date().toString(), user.email]]
                    },
                    auth: oauth2Client
                  },
                  (err, response) => {
                ...
                confirmation.needsReimbursement === "0" &&
                result.values.some(value => {
                  return (
                    JSON.stringify(value) === JSON.stringify(user.email.split())
                  );
  ...
  router.get("/resume/:id", isOwnerOrAdmin, (req, res) => {
    ...
    s3.headObject(s3Params, (err, data) => {
      if (err) {
        ...
        var token = jwt.sign({ fileName: fileName }, JWT_SECRET, {
          expiresIn: "30s"
        });
        res.json({
          token: token
