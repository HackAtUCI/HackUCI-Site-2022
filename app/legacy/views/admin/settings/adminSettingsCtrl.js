angular.module("reg").controller("AdminSettingsCtrl", [
  ...
  function($scope, $sce, SettingsService) {
    $scope.settings = {};
    SettingsService.getPublicSettings().success(function(settings) {
    ...
    $scope.updateAllowMinors = function() {
      SettingsService.updateAllowMinors($scope.settings.allowMinors).success(
        function(data) {
    ...
    SettingsService.getWhitelistedEmails().success(function(emails) {
      $scope.whitelist = emails.join(", ");
    });

    $scope.updateWhitelist = function() {
      SettingsService.updateWhitelistedEmails(
        $scope.whitelist.replace(/ /g, "").split(",")
    ...
    $scope.updateRegistrationTimes = function() {
      ...
      SettingsService.updateRegistrationTimes(open, close).success(function(
        settings
      ) {
        updateSettings(settings);
    ...
    $scope.updateConfirmationTime = function() {
      ...
      SettingsService.updateConfirmationTime(confirmBy).success(function(
        settings
      ) {
        updateSettings(settings);
    ...
    $scope.updateWaitlistText = function() {
      ...
      SettingsService.updateWaitlistText(text).success(function(data) {
    ...
    $scope.updateAcceptanceText = function() {
      ...
      SettingsService.updateAcceptanceText(text).success(function(data) {
    ...
    $scope.updateConfirmationText = function() {
      ...
      SettingsService.updateConfirmationText(text).success(function(data) {
